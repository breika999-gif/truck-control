...
import CapabilitiesSheet, { type CommandTarget } from './CapabilitiesSheet';
...
interface ChatPanelProps {
  ...
  handleChat: (textOverride?: string) => void;
  isRecording: boolean;
  handleMicStart: () => void;
  handleMicStop: () => void;
  kbHeight: number;
  ...
  insets: EdgeInsets;
  micLoading?: boolean;
  onTargetedQuickAction?: (text: string, target: CommandTarget) => void | Promise<void>;
  onClose: () => void;
}
...
const ChatPanel: React.FC<ChatPanelProps> = memo(({
  ...
  const quickActionMode = gptChatOpen ? 'gpt' : 'gemini';
  ...
  return (
    <>
      ...
      >
        ...
        <ChatQuickActions mode={quickActionMode} onSelect={handleQuickAction} />
        <View style={[styles.chatInputRow, { paddingBottom: 12 }]}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <Icon name="lightning-bolt" size={20} color="#ffcc00" />
          </TouchableOpacity>
          <TextInput
            ref={inputRef}
      ...
      <CapabilitiesSheet
        ...
        onSelect={(label, target) => {
          setCapabilitiesOpen(false);
          if (onTargetedQuickAction) {
            void onTargetedQuickAction(label, target);
          } else {
